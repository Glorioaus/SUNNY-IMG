...
async function renameByOrder() {
    ...
    if (sourceImages.length !== CONFIG.totalEmployees) {
        log(`警告: 图片数量(${sourceImages.length})与员工数(${CONFIG.totalEmployees})不匹配`, 'warning');
    ...
    log(`\n完成！成功处理 ${successCount}/${sourceImages.length} 张图片`, 'success');
}
...
async function checkAndFillMissing() {
    ...
    const existingFiles = new Set(
        fs.readdirSync(CONFIG.targetFolder)
    ...
    log(`缺失: ${missingIds.length}/${CONFIG.totalEmployees}`, 
